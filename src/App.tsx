...
function App() {
  const [tablesData, setTablesData] = useState<any>({ flavanoidsData: [], gammaData: [] });
  ...
  function getTablesData() {
    ...
    wineData.forEach((wine: any) => {
      const indexFlavanoids = flavanoidsData.findIndex((item: any) => item.alcohol === wine.Alcohol);
      const indexGamma = gammaData.findIndex((item: any) => item.alcohol === wine.Alcohol);
      ...
      if (indexFlavanoids >= 0) {
        ...
        flavanoidsData[indexFlavanoids].counts[wine.Flavanoids] = (flavanoidsData[indexFlavanoids].counts[wine.Flavanoids] || 0) + 1;
      } else {
      ...
      if (indexGamma >= 0) {
        ...
        gammaData[indexGamma].counts[gamma] = (gammaData[indexGamma].counts[gamma] || 0) + 1;
      } else {
